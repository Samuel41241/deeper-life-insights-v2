...
import { BellRing, AlertTriangle, Eye, Heart, TrendingDown, Filter, Search, ChevronRight } from "lucide-react";
import { useEngagementData, type RiskLevel, type MemberEngagement } from "@/hooks/use-engagement";
...
const riskConfig: Record<RiskLevel, { label: string; color: string; icon: typeof BellRing }> = {
  no_concern: { label: "No Concern", color: "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400", icon: Eye },
  watchlist: { label: "Watchlist", color: "bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400", icon: Eye },
  follow_up: { label: "Follow-Up Required", color: "bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-400", icon: AlertTriangle },
  pastoral_attention: { label: "Pastoral Attention", color: "bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400", icon: Heart },
};
...
function RiskBadge({ level }: { level: RiskLevel }) {
  ...
  return <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-semibold ${cfg.color}`}>{cfg.label}</span>;
}

function TrendBadge({ trend }: { trend: string }) {
  if (trend === "declining") return <span className="inline-flex items-center gap-1 text-xs font-medium text-destructive"><TrendingDown className="h-3 w-3" />Declining</span>;
  if (trend === "improving") return <span className="inline-flex items-center gap-1 text-xs font-medium text-emerald-600">↑ Improving</span>;
  return <span className="text-xs text-muted-foreground">Stable</span>;
}

export default function Engagement() {
  const { data, isLoading } = useEngagementData();
  ...
  const filtered = useMemo(() => {
    ...
    return list.sort((a, b) => {
      const order: Record<RiskLevel, number> = { pastoral_attention: 0, follow_up: 1, watchlist: 2, no_concern: 3 };
  ...
  return (
    <div className="admin-page">
      <div>
        ...
        <p className="admin-page-description">Track member engagement, flag absentees, and surface intervention needs</p>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {([
          ...
          <div key={c.label} className={`stat-card ${c.color}`}>
            ...
            <p className="text-2xl font-heading font-bold mt-1">{isLoading ? "—" : c.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <Input placeholder="Search member..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 h-10" />
        </div>
        <Select value={locationFilter} onValueChange={setLocationFilter}>
          <SelectTrigger className="w-[180px] h-10"><SelectValue placeholder="All Locations" /></SelectTrigger>
          <SelectContent>
        ...
        <Select value={categoryFilter} onValueChange={setCategoryFilter}>
          <SelectTrigger className="w-[160px] h-10"><SelectValue placeholder="All Categories" /></SelectTrigger>
          <SelectContent>
      ...
      {isLoading ? (
        <div className="space-y-3">
          {[1, 2, 3].map((i) => <div key={i} className="stat-card h-20 animate-pulse" />)}
        </div>
      ) : filtered.length === 0 ? (
...
function MemberRow({ member: m }: { member: MemberEngagement }) {
  return (
    <Link to={`/admin/members/${m.member_id}`} className="stat-card flex items-center gap-4 hover:bg-accent/30 transition-colors cursor-pointer block">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-0.5 mt-1 text-xs text-muted-foreground">
          ...
          <span>Attended {m.attended_count}/{m.total_services_in_window}</span>
          <span>{m.attendance_rate}% rate</span>
          {m.last_attended && <span>Last: {m.last_attended}</span>}
          {m.consecutive_absences > 0 && <span className="text-destructive font-medium">{m.consecutive_absences} consecutive absences</span>}
        </div>
      </div>
