...
import { QrCode, Search, CreditCard } from "lucide-react";
...
export default function QRCards() {
  ...
  const filtered = cards.data?.filter((c) => {
    if (!search) return true;
    const name = (c as any).members?.full_name || "";
    return name.toLowerCase().includes(search.toLowerCase()) || c.card_number.toLowerCase().includes(search.toLowerCase());
  });

  const handleStatusChange = async (cardId: string, status: string) => {
    try {
      await updateStatus.mutateAsync({ id: cardId, status: status as Enums<"card_status"> });
  ...
  return (
    <div className="admin-page">
      ...
      <div className="relative max-w-sm">
        ...
        <Input placeholder="Search by name or card number..." className="pl-9 h-11" value={search} onChange={(e) => setSearch(e.target.value)} />
      </div>
          ...
          <tbody>
            {filtered?.map((c) => (
              <tr key={c.id} className="border-b last:border-0 hover:bg-muted/50">
                ...
                <td className="py-3 text-muted-foreground">{(c as any).members?.locations?.name || "—"}</td>
                <td className="py-3 text-muted-foreground">{c.issued_date}</td>
                <td className="py-3">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${statusColors[c.status] || ""}`}>{c.status}</span>
                </td>
                <td className="py-3">
          ...
          <div key={c.id} className="stat-card">
            <div className="flex items-start justify-between gap-2">
              <div>
                ...
                <p className="text-xs text-muted-foreground">{(c as any).members?.locations?.name || "—"} · {c.issued_date}</p>
              </div>
              <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium shrink-0 ${statusColors[c.status] || ""}`}>{c.status}</span>
            </div>
            <div className="mt-3">
              <Select value={c.status} onValueChange={(v) => handleStatusChange(c.id, v)}>
